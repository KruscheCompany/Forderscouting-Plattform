<template>
  <q-table :data="data" :columns="columns" row-key="title" :filter="query"
    class="bg-white shadow-1 radius-20 -mb-xl pagination-no-shadow" :pagination="{
      ...
      page: 1,
      rowsPerPage: 15
    }" :loading="loading" color="primary" rows-per-page-label="Datensätze pro Seite:"
    no-data-label="Keine Daten verfügbar" no-results-label="Keine passenden Datensätze gefunden">
    <template v-slot:top>
      <div class="col-12 col-md-3">
        <q-input borderless outlined class="input-radius-6 no-shadow q-mb-sm q-mt-sm" v-model="query"
          placeholder="Suche" dense role="searcbox">
