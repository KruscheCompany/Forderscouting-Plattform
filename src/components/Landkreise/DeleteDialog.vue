            ...
            <q-card-section>
                <div class="row justify-center q-ml-lg ">
                    ...
                    <q-btn :label="$t('approve')" type="submit" unelevated size="16px" color="red" no-caps
                        class="no-shadow radius-6 q-px-xl" :loading="isLoading" @click="deleteLandkreis" />
                </div>
            </q-card-section>
        </q-card>
    </q-dialog>
...
export default {
    ...
    methods: {
        async deleteLandkreis() {
            if (!!this.id) {
                this.isLoading = true;
                const res = await this.$store.dispatch(
                    "landkreis/deleteLandkreis",
                    { id: this.id }
                );
