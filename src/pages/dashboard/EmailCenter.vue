<template>
  <q-page class="q-mt-lg" :class="$q.screen.gt.sm ? 'q-mx-xl' : 'q-mx-sm'">
      ...
      <div class="col-12">
        <div class="row q-col-gutter-md justify-end">
          <div class="col-12 col-md-auto">
            <q-btn color="blue" icon="add" unelevated outline class="q-mb-md radius-6 text-weight-600"
              :class="{ 'full-width': $q.screen.lt.md }" no-caps @click="dialogState = true">
            ...
            <q-dialog v-model="$_options">
              <q-card class="q-px-md q-py-xs radius-10"
                :style="$q.screen.lt.sm ? 'width: 95vw; max-width: 95vw;' : 'min-width: 70%'">
                <h5 style="margin: 5px 0 15px 5px">
                  {{ $t("sendEmail") }}
                </h5>
                <q-card-section>
                  <q-select v-model="form.userGroup" behavior="default" :options="userGroupsOptions"
                    :label="$t('generalData.selectUserGroup')" outlined :rules="[(val) => !!val || $t('Required')]"
                    dense class="no-shadow radius-6" multiple use-chips />
                </q-card-section>

                <!-- Email subject -->
                <q-card-section>
                  <q-input borderless outlined class="input-radius-6 no-shadow q-mb-sm q-mt-sm" v-model="form.subject"
                    :rules="[(val) => !!val || $t('Required')]" :placeholder="$t('Subject')" dense>
                  </q-input>
                </q-card-section>

                <!-- Email body -->
                <q-card-section>
                  <q-editor borderless outlined class="input-radius-6 no-shadow q-mb-sm q-mt-sm" v-model="form.body"
                    :rules="[(val) => !!val || $t('Required')]" type="textarea" :dense="$q.screen.lt.md" :toolbar="[
                      [
                        {
                ...
                <!-- Email attachments -->
                <q-card-section>
                  <q-file flat v-model="form.attachments" class="uploadInput input-radius-6 text-white"
                    label-color="white" dark bg-color="primary" :label="$t('attachFiles')" multiple display-value=""
                    append>
                ...
                <q-card-section>
                  <div class="row q-col-gutter-sm justify-center">
                    <div class="col-12 col-sm-auto">
                      <q-btn :label="$t('category&Keyword.cancel')" outline v-close-popup size="14px" color="primary"
                        no-caps class="no-shadow radius-6 q-px-md full-width" :loading="isLoading"
                        @click="cancelDialog()" />
                    </div>
                    <div class="col-12 col-sm-auto">
                      <q-btn :label="$t('confirm')" unelevated :loading="isLoading" size="14px" color="primary" no-caps
                        class="no-shadow radius-6 q-px-md q-py-sm full-width" @click="sendEmail()" />
                    </div>
                  </div>
                </q-card-section>
              </q-card>
            ...
            <q-dialog v-model="$_viewOptions" v-if="viewDialogState">
              <q-card class="q-px-md q-py-xs radius-10" style="padding: 20px; min-width: 70%">
                ...
                <q-card-section>
                  ...
                  <q-list>
                    <q-item v-for="(attachment, index) in viewDetails.attachments.data" :key="index">
                      <q-item-section>
                        <q-item-label>
                          <a class="q-mb-sm text-blue block text-weight-600 font-14" target="_blank"
                            rel="noopener noreferrer" :href="`${appUrl}${attachment.attributes.url}`">{{
                ...
                <q-card-section>
                  <div class="row justify-center">
                    <q-btn :label="$t('Close')" outline v-close-popup size="14px" color="primary" no-caps
                      class="no-shadow radius-6 q-px-md q-mr-sm" :loading="isLoading" @click="cancelViewDialog()" />
                  </div>
                </q-card-section>
              </q-card>
            </q-dialog>
    ...
    <!-- Emails table  -->
    <q-table class="radius-20 shadow-1 pagination-no-shadow" :title="$t('E-Mails')" :data="this.emails"
        ...
        <q-tr :props="props">
          <q-td @click="view(props.row)" auto-width v-for="col in props.cols" :key="col.name" :props="props"
            ...
            <span v-if="col.name === 'status'">
              <q-chip :label="props.row[col.name] === true ? $t('Sent') : $t('NotSent')" class="text-white"
