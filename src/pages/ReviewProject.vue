        ...
        <template v-if="project">
          ...
          <q-card class="review-card shadow-2">
            ...
            <q-card-section v-if="!alreadyAnswered && !submitted">
              <VorpruefungDecisionForm :submitting="submitting" @submit="submit" />
            </q-card-section>
...
export default {
  ...
  computed: {
    ...
    resolvedStatus() {
      if (this.submitted) return this.submittedDecision;
  ...
  methods: {
    ...
    async submit(payload) {
      ...
      try {
        await api.post(`/api/vorpruefung-tickets/by-token/${this.$route.params.token}/respond`, payload);
        this.submittedDecision = payload.decisionType;
        this.submitted = true;
        this.ticket = { ...this.ticket, responseText: payload.responseText };
      } catch (error) {
